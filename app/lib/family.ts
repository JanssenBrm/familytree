...
const calculateAge = (members: PersonBase[]): Person[] =>
  members.map((m) => {
    return {
      ...m,
      age: getAge(m),
...
export const getFamilies = async (userId: number): Promise<Family[]> => {
  try {
    const result = await sql<Family>`
    SELECT fams.id, fams.name
    FROM family_managers_family_rel as rel INNER JOIN family_families as fams ON rel.familyid = fams.id
    WHERE managerid = ${userId}
    `;

    return result.rows;
  } catch (error) {
    console.error(`Failed to fetch families for user ${userId}:`, error);
...
export const deleteMarriage = async (
  family: number,
  id: number
): Promise<void> => {
  try {

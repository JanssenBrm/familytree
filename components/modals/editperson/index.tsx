...
const EditPersonModal = ({
  ...
  const getPerson = (id: number): Person | undefined =>
    members.find((p: Person) => p.id === id);
  ...
  const marriageLabels = marriages.map((m: Marriage) => {
    ...
    return {
      id: m.id,
      label: `${p1?.firstname || "Onbekend"} ${p1?.lastname || "Onbekend"} - ${
        p2?.firstname || "Onbekend"
      } ${p2?.lastname || "Onbekend"}`,
    };
  });

  const getMarriage = (id?: number): Marriage | undefined =>
    marriages.find((m: Marriage) => m.p1 === id || m.p2 === id);
    ...
    if (person) {
      ...
      if (marriage) {
        return {
          ...
          date: marriage.date
            ? parseDate(moment(marriage.date).format("YYYY-MM-DD"))
            : undefined,
          city: marriage.city,
        };
      }
  ...
  const getChild = (id?: number): Child | undefined =>
    childList.find((c: Child) => c.childid === id);
    ...
    defaultValues: {
      person: {
        ...
        birthdate: person?.birthdate
          ? parseDate(
              moment(
                person.birthdate.length === 4
                  ? `${person.birthdate}/01/01`
                  : person.birthdate
              ).format("YYYY-MM-DD")
            )
          : undefined,
        ...
        deathdate: person?.deathdate
          ? parseDate(
              moment(
                person.deathdate.length === 4
                  ? `${person.deathdate}/01/01`
                  : person.deathdate
              ).format("YYYY-MM-DD")
            )
          : undefined,
  ...
  const submitMarriage = async (person: Person) => {
    if (formState.isValid && familyId) {
      ...
      if (
        existingMarriage &&
        existingMarriage.id &&
        (existingMarriage.p1 === partner || existingMarriage.p2 === partner) &&
        existingMarriage.date === marriageDate &&
        existingMarriage.city === city
      ) {
        ...
        const updatedMarriage: Marriage = await updateMarriage(
          familyId,
          existingMarriage.id,
          {
            ...existingMarriage,
        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1">
              {person
                ? `Familielid ${person?.firstname} ${person?.lastname} bewerken`
            ...
            <ModalBody className="flex flex-col overflow-auto">
              ...
              />
              <h3 className="p-2 font-bold text-neutral-500 uppercase text-sm">
                Geboorte
              </h3>
              <Controller
              ...
              <Controller
                render={({ field }) => (
                  ...
                  >
                    {[
                      { id: 0, firstname: "Onbekend", lastname: "" },
                      ...members,
                    ]
                      .map((m) => ({
                        ...m,
              ...
              <Controller
                render={({ field }) => (
                  ...
                  <Autocomplete
                    {...field}
                    label="Kind van"
                    placeholder="Selecteer huwelijk"
                    onSelectionChange={(key: any) => field.onChange(+key)}
                    selectedKey={`${field.value}`}
                  >

...
const FamilyTree = () => {
    ...
    useEffect(() => {
        if (search !== '') {
            setSearchResults(nodes.filter(n => (n.data as Person)?.firstname?.toLowerCase().includes(search.toLowerCase()) ||
                (n.data as Person)?.lastname?.toLowerCase().includes(search.toLowerCase()) ||
                (n.data as Person)?.comments?.toLowerCase().includes(search.toLowerCase())
            ));
        } else {
            setSearchResults([]);
        }
            ...
            <div className="absolute top-2 right-2 z-50 left-2 w-[96%] md:w-80 md:left-auto">
                <div className="relative">
                    ...
                    <input type="search" id="default-search"
                           ...
                           placeholder="Zoeken..." onKeyUp={(evt) => setSearch((evt.target as any).value)}/>
                </div>
                {
                    search && search !== '' && (
